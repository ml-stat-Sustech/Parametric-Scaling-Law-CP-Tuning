...
class RAPS(APS):
    def __init__(self, penalty=0.01, kreg=1):
    ...
    def _calculate_all_label(self, probs):
        ...
        reg = torch.maximum(self.penalty * (torch.arange(1, probs.shape[-1] + 1, device=probs.device) - self.kreg),
    ...
    def _calculate_single_label(self, probs, label):
        ...
        reg = torch.maximum(self.penalty * (idx[1] + 1 - self.kreg), torch.tensor(0).to(probs.device))
