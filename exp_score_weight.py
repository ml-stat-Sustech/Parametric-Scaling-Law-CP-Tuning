...
from predictor.weight_predictor import WeightPredictor

def search_weight(loader, model, alpha, device):
    ...
    for a in np.arange(0, 1.1, step_size):
        for b in np.arange(0, 1.1, step_size):
            c = 1 - a - b
            if 0 <= c <= 1:
                combinations.append([a, b, c])
                
    for para in combinations:
        wpredictor = WeightPredictor(model, alpha, para, device)
        
        wpredictor.calibrate(loader)
        
        acc, cov, size = wpredictor.evaluate(loader)
...
def run_experiment(model_name, cal_num, alpha, num_runs, device):
    ...
    for run in range(num_runs):
        ...
        optimal_para = search_weight(cp_calibloader, model, alpha, device)
        wpredictor = WeightPredictor(model, alpha, optimal_para, device)

        optimal_para_dz = search_weight(tuning_calibloader, model, alpha, device)
        wpredictor_dz = WeightPredictor(model, alpha, optimal_para_dz, device)

    
        # Calibration using cp_calibloader
        wpredictor.calibrate(cp_calibloader)
        acc, cov, size = wpredictor.evaluate(testloader)

        wpredictor_dz.calibrate(cp_calibloader)
        acc_dz, cov_dz, size_dz = wpredictor_dz.evaluate(testloader)
